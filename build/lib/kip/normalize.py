...
def _assets_for(path: Path, source_id: str, normalizer: str | None, *,
    ...
    try:
        ...
        if normalizer == "text_v1" and path.suffix.lower() in {".md", ".markdown"}:
            # Markdown is the format most documents arrive in that nobody thinks
            # of as a format, and it was the one common input with no asset path
            # at all: a comparison table reached the corpus as a flattened row
            # with no grid behind it.
            from .md_assets import markdown_assets
            return markdown_assets(text, source_id)
