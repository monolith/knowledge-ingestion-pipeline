...
class HandoffClient:
    ...
    def _portable(self, image: str) -> str:
        """An image path relative to the run directory, when it is inside it."""
        run_dir = self.root.parent
        try:
            return str(Path(image).resolve().relative_to(run_dir.resolve()))
        except ValueError:
            return str(image)
    ...
    def complete_json(
        ...
        *,
        ...
        #
        # Identity is the image's CONTENT, not where it happens to sit. The path
        # was used at first and it made a run unreplayable anywhere but the
        # machine that produced it: the recorded id folded in an absolute path,
        ...
        request = {
            ...
            "system": system,
            "user": user,
            # Recorded relative to the run, so the record is portable and does
            # not carry the producing machine's directory layout. The digest is
            # what the id is built from; the path is how a reader finds the file.
            "images": [self._portable(image) for image in (images or [])],
...
def _image_digest(image: str) -> str:
    """A short content hash of an image file.

    Falls back to the file NAME, never the full path, when the bytes cannot be
    read: a name is at least the same on every machine, where an absolute path
    is not.
    """
    import hashlib

    try:
        return hashlib.sha256(Path(image).read_bytes()).hexdigest()[:16]
    except OSError:
