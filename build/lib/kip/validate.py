...
def validate_run(ctx: RunContext) -> dict[str, Any]:
    ...
    # A run produced before anchoring existed has assets and no links file. Every
    # asset would then look orphaned, which is not a finding about that run -- it
    # is this check being applied to a run that never had the chance to satisfy
    # it. Reporting 119 orphans where the real answer is "linking never ran" is
    # the kind of false alarm that teaches a reader to ignore the warning.
    ...
    if orphaned and not linking_ran:
        warnings.append(
